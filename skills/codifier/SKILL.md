---
name: codifier
description: Collect the decisions already settled in a codebase and write them to docs/codifier/. Calls the four workers, then writes. Use when a project has decisions buried in its code and history that were never recorded.
---
...
Read `TEMPLATE.md` in this directory for the form a decision takes.
...
3. codifier-escalate  settled set, index -> granularity per decision
...
- One decision per file, all in one flat directory. No subdirectory per granularity
- A file name never carries the granularity - escalation must not move a file
- A decision file a person wrote is never overwritten. On a name collision, report
  and stop
...
paths this decision rests on, separated by `;`. In-degree is not a column - count
the `grounds` column instead.

Rebuild it from the decision files alone. Deleting it must lose nothing.

## Timecode

Take what `codifier-collect` returns and write it into the decision file. **Never
...
On reaching something the decisions do not settle, stop without writing it and say
what is missing. Filling that gap is a decision, and decisions are not made here.

## Language

Write decisions in the language the project's existing documents use. Where none
