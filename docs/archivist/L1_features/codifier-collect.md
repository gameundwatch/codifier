...
条項はここで二つに分かれる。捨てた案を持つものが decision で、履歴から徴候で判定する。
持たないものが definition で、名前と対象を結びつける行為そのものがそれにあたる。
外から名指しされている識別子は、変えれば名指ししている全箇所が壊れるため、既に決まって
いる。この分だけコードが指す条項の写像が書ける。

## Availability

- 走査範囲を渡すと、その範囲のコードと履歴から条項の候補が挙がる
    - 範囲が決まるまで走査は始まらない
    - 件数が先に示される
...
- 公開された識別子については、名前ごとに命名の条項が挙がる
    - 名前、対象、どこから名指しできるかが書かれる
    - 同じ概念に別の綴りが併存していれば、その併存も書かれる
    - 命名規則としてまとめられることはない
- 挙がらなかったものが理由つきで残る
- 途中で問い合わせが来ない
- 候補には条項が成立したコミットの時刻が付く
